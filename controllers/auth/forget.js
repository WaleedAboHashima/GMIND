  ...
  try {
    if (!emailRegex.test(email)) {
      ...
      if (!findOne) {
        ...
        if (!OTP) {
          res.status(500).json({ success: true, message: "Failed to send the email" });
        } else {
          res.sendStatus(200);
        }
      }
...
exports.VerifyHuman = asyncHandler(async (req, res) => {
  ...
  try {
    if (!emailRegex.test(email)) {
      ...
      if (!OTP) {
        res.status(500).json({ success: false, message: "Failed to send the email", OTP });
      } else {
        res.sendStatus(200);
      }
    }
...
exports.OTPCheck = asyncHandler(async (req, res) => {
  ...
  if (!email || !OTP)
    ...
    try {
      ...
      if (!user) {
        res.status(404).json({ success: false, message: "Can't Find Your Email." });
      } else {
        if (!validate) {
          res.status(403).json({ success: false, message: "Invalid Otp" });
        } else {
